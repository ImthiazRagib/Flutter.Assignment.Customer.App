...
import 'package:assignment_customer_app/features/profile/profile.dart';
  ...
  routes: [
    ShellRoute(
      builder: (context, state, child) =>
          Stack(children: [child, const FloatingNavBar()]),
      redirect: (context, state) {
        final authProvider = context.read<AuthProvider>();
        final loggedIn = authProvider.isLoggedIn;

        final path = state.matchedLocation;
        final isLoginRoute = path == '/login' || path == '/signup';
        final isProtectedRoute = path == '/dashboard' || path == '/profile';
